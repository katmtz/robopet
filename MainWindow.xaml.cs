...
namespace Microsoft.Samples.Kinect.FaceBasics
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        private bool handClosed, handOpen, engaged, happy, up, down, left, right, lasso;
        ...
        public MainWindow()
        {
            ...
            // initialize motion reference point
            this.ReferencePos = new Point(150, 150);
            ...
            using (FaceFrame faceFrame = e.FrameReference.AcquireFrame())
            {
                if (faceFrame != null)
                {
                    ...
                    if (this.ValidateFaceBoxAndPoints(faceFrame.FaceFrameResult))
                    {
                        ...
                        // FACE FRAME CASES
                        // - set control signals every frame
                        FaceFrameResult result = faceFrame.FaceFrameResult;

                        this.happy = (result.FaceProperties[FaceProperty.Happy].ToString() == "Yes") ? true : false;
                        this.engaged = (result.FaceProperties[FaceProperty.Engaged].ToString() == "Yes") ? true : false;
                        ...
                        if (body != null)
                        {
                            if (body.IsTracked)
                            {
                                ...
                                switch (body.HandRightState)
                                {
                                    ...
                                    if (this.handOpen)
                                    {
                                        // set hand position as reference position.
                                        CameraSpacePoint CurrPos = handRight.Position;
                                        double dx = CurrPos.X - this.ReferencePos.X;
                                        double dy = CurrPos.Y - this.ReferencePos.Y;
                                        if (dx < 10)
                                        {
                                            this.right = true;
                                        }
                                    ...
                                    else if (this.handClosed)
                                    {
                                        CameraSpacePoint spacePos = handRight.Position;
                                        this.ReferencePos.X = spacePos.X;
                                        this.ReferencePos.Y = spacePos.Y;
                                    }
        ...
        {
            // HIJACKING THIS FUNCTION FOR SERIAL PURPOSES!!!!!!!!
            if (this.engaged)
            {
                if (this.handOpen)
                {
                    this.WriteToSerial(this.Serial, "1"); // ATTN
                    Debug.Print("STATE: ATTN");
                }
                if (this.up)
                {
                    this.WriteToSerial(this.Serial, "2"); // FORWARD
                    Debug.Print("STATE: FORWARD");
                }
                else if (this.down)
                {
                    this.WriteToSerial(this.Serial, "3"); // BACKWARD
                    Debug.Print("STATE: BACKWARD");
                }
                if (this.left)
                {
                    this.WriteToSerial(this.Serial, "4"); // LEFT
                    Debug.Print("STATE: LEFT");
                }
                else if (this.right)
                {
                    this.WriteToSerial(this.Serial, "5"); // RIGHT
                    Debug.Print("STATE: RIGHT");
                }
                if (this.happy)
                {
                    this.WriteToSerial(this.Serial, "6"); // HAPPY
                    Debug.Print("STATE: HAPPY");
                }
                if (this.lasso)
                {
                    this.WriteToSerial(this.Serial, "7"); // TRICK
                    Debug.Print("STATE: TRICK");
                }
            }
            else
            {
                this.WriteToSerial(this.Serial, "0"); // IDLE
                Debug.Print("STATE: IDLE");
            }
        ...
        // if serial port is not open, open port
        // send a string code out to serial
        private void WriteToSerial(SerialPort sp, string code)
        {
